...
public class K_S {
	...
	//得到比较模型的数据
    private static void GetData(List<Double> res, List<Double> y) {
        KS=0;
        for (int j = 1; j <res.size() ; j++) {
            double sub = abs(res.get(j)-y.get(j));
            if(KS<sub)
                KS=sub;
        }
    }
    ...
    //置信区间在95%的K-S检验
    public static boolean isKSTest95( int fn) {
        if (fn > 100) {
    ...
    {
        System.out.println("KS值为："+KS);
        if(result)
            System.out.println("在显著性水平为0.05的情况下,接受原假设，即检验样本满足该模型。");
        else
            System.out.println("在显著性水平为0.05的情况下,拒绝原假设，即检验样本不满足该模型。");
    }
    
    //运行K-S检验
    public  static  String run(List<Double> res, List<Double> y ) {
        GetData(res,y);
        boolean re=isKSTest95(res.size()-1);
        showResult(re);
        return  GetText(re);
    }
    
    //返回结果
    private  static String GetText(boolean result)
    {
        String re;
        if(result)
            re="  Accept this Model";
        else
            re="  Reject this Model";
        return "KS Distance="+KS+re;
    }
}
